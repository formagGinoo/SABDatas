...
function rpcs:Role_Report(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Role_Report_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Role_Report timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Role_Report_SC)
...
function rpcs:Role_ReportNoCB(sdpMsg, timeout, retryCount)
  self:Role_Report(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Role_Report_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Role_Report(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Role_Report(cb, tag)
  if cb == nil then
    log.info("ListenRole_Report cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Role_Report_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Role_Report_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Role_Report_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Role_Report_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Role_Report(handler)
