...
function rpcs:Item_GetList(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Item_GetList_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Item_GetList timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Item_GetList_SC)
...
function rpcs:Item_GetListNoCB(sdpMsg, timeout, retryCount)
  self:Item_GetList(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Item_GetList_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Item_GetList(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Item_GetList(cb, tag)
  if cb == nil then
    log.info("ListenItem_GetList cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Item_GetList_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Item_GetList_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Item_GetList_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Item_GetList_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Item_GetList(handler)
