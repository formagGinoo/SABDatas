...
function rpcs:Role_SetCard(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Role_SetCard_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Role_SetCard timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Role_SetCard_SC)
...
function rpcs:Role_SetCardNoCB(sdpMsg, timeout, retryCount)
  self:Role_SetCard(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Role_SetCard_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Role_SetCard(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Role_SetCard(cb, tag)
  if cb == nil then
    log.info("ListenRole_SetCard cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Role_SetCard_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Role_SetCard_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Role_SetCard_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Role_SetCard_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Role_SetCard(handler)
