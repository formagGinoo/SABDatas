...
function rpcs:Shop_Refresh(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Shop_Refresh_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Shop_Refresh timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Shop_Refresh_SC)
...
function rpcs:Shop_RefreshNoCB(sdpMsg, timeout, retryCount)
  self:Shop_Refresh(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Shop_Refresh_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Shop_Refresh(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Shop_Refresh(cb, tag)
  if cb == nil then
    log.info("ListenShop_Refresh cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Shop_Refresh_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Shop_Refresh_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Shop_Refresh_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Shop_Refresh_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Shop_Refresh(handler)
