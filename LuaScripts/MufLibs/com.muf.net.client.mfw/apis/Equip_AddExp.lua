...
function rpcs:Equip_AddExp(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Equip_AddExp_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Equip_AddExp timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Equip_AddExp_SC)
...
function rpcs:Equip_AddExpNoCB(sdpMsg, timeout, retryCount)
  self:Equip_AddExp(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Equip_AddExp_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Equip_AddExp(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Equip_AddExp(cb, tag)
  if cb == nil then
    log.info("ListenEquip_AddExp cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Equip_AddExp_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Equip_AddExp_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Equip_AddExp_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Equip_AddExp_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Equip_AddExp(handler)
