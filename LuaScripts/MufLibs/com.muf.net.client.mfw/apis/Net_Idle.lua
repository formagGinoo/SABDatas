...
function rpcs:Net_Idle(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Net_Idle_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Net_Idle timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Net_Idle_SC)
...
function rpcs:Net_IdleNoCB(sdpMsg, timeout, retryCount)
  self:Net_Idle(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Net_Idle_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Net_Idle(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Net_Idle(cb, tag)
  if cb == nil then
    log.info("ListenNet_Idle cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Net_Idle_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Net_Idle_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Net_Idle_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Net_Idle_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Net_Idle(handler)
