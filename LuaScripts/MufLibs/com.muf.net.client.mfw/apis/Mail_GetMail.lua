...
function rpcs:Mail_GetMail(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Mail_GetMail_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Mail_GetMail timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Mail_GetMail_SC)
...
function rpcs:Mail_GetMailNoCB(sdpMsg, timeout, retryCount)
  self:Mail_GetMail(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Mail_GetMail_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Mail_GetMail(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Mail_GetMail(cb, tag)
  if cb == nil then
    log.info("ListenMail_GetMail cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Mail_GetMail_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Mail_GetMail_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Mail_GetMail_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Mail_GetMail_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Mail_GetMail(handler)
