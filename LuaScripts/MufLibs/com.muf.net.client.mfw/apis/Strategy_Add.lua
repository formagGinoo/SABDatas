...
function rpcs:Strategy_Add(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Strategy_Add_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Strategy_Add timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Strategy_Add_SC)
...
function rpcs:Strategy_AddNoCB(sdpMsg, timeout, retryCount)
  self:Strategy_Add(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Strategy_Add_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Strategy_Add(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Strategy_Add(cb, tag)
  if cb == nil then
    log.info("ListenStrategy_Add cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Strategy_Add_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Strategy_Add_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Strategy_Add_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Strategy_Add_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Strategy_Add(handler)
