...
function rpcs:Chat_GetChat(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Chat_GetChat_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Chat_GetChat timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Chat_GetChat_SC)
...
function rpcs:Chat_GetChatNoCB(sdpMsg, timeout, retryCount)
  self:Chat_GetChat(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Chat_GetChat_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Chat_GetChat(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Chat_GetChat(cb, tag)
  if cb == nil then
    log.info("ListenChat_GetChat cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Chat_GetChat_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Chat_GetChat_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Chat_GetChat_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Chat_GetChat_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Chat_GetChat(handler)
