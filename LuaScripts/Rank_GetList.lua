...
function rpcs:Rank_GetList(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Rank_GetList_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Rank_GetList timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Rank_GetList_SC)
...
function rpcs:Rank_GetListNoCB(sdpMsg, timeout, retryCount)
  self:Rank_GetList(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Rank_GetList_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Rank_GetList(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Rank_GetList(cb, tag)
  if cb == nil then
    log.info("ListenRank_GetList cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Rank_GetList_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Rank_GetList_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Rank_GetList_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Rank_GetList_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Rank_GetList(handler)
