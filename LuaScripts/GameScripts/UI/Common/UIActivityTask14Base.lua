local UIActivityTask14Base = class("UIActivityTask14Base", require("UI/Common/UIBase"))
...
function UIActivityTask14Base:AfterInit()
  UIActivityTask14Base.super.AfterInit(self)
...
function UIActivityTask14Base:CreateBoxItem(itemObj, itemId, itemNum)
  if not itemObj then
    return nil
  end
...
function UIActivityTask14Base:OnInactive()
  UIActivityTask14Base.super.OnInactive(self)
...
function UIActivityTask14Base:RemoveEventListeners()
...
function UIActivityTask14Base:OnEventUpdateQuest(tParam)
...
function UIActivityTask14Base:OnEventTakeQuestReward(sc)
...
function UIActivityTask14Base:OnEventTakeDailyReward(sc)
...
function UIActivityTask14Base:OnEventTakeFinalReward(sc)
  if sc.iActivityId ~= self.m_stActivity:getID() then
    return
  end
...
function UIActivityTask14Base:RefreshDailyTaskTab(bInit)
...
function UIActivityTask14Base:GetDayQuestState(iDay)
  local bState
...
function UIActivityTask14Base:RefreshDailyReward()
...
function UIActivityTask14Base:OnItemIconClicked(iID, iNum)
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  utils.openItemDetailPop({iID = iID, iNum = iNum})
end

function UIActivityTask14Base:OnClaimDailyRewardClicked()
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  self.m_stActivity:RequestTakeDailyReward(self.iSelectedDay)
end

function UIActivityTask14Base:RefreshFinalReward()
...
function UIActivityTask14Base:RefreshPointReward()
  local iScoreCur = self.m_stActivity:GetScore()
...
function UIActivityTask14Base:OnPointItemClick(item)
  self.m_stActivity:RequestTakeFinalReward(item.score)
end

function UIActivityTask14Base:GetPointReward()
  self.m_stActivity:RequestTakeFinalReward(self.m_iFinalRewardScoreNext)
end

function UIActivityTask14Base:RefreshRemainTime()
...
function UIActivityTask14Base:refreshTaskLoopScroll(is_tab_change)
...
function UIActivityTask14Base:updateScrollViewCell(index, cell_object, cell_data)
...
function UIActivityTask14Base:OnBtngetallClicked()
...
function UIActivityTask14Base:OnBtnfinalrewardreceiveClicked()
  self.m_stActivity:RequestTakeFinalReward(self.m_iFinalRewardScoreNext)
end

function UIActivityTask14Base:OnTabItemClk(idx)
...
function UIActivityTask14Base:Back()
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  self:CloseForm()
  self.iSelectedDay = nil
end

function UIActivityTask14Base:OnDestroy()
  UIActivityTask14Base.super.OnDestroy(self)
  self:CheckRecycleSpine(true)
end

function UIActivityTask14Base:CheckRecycleSpine(isResetParam)
  if self.m_HeroSpineDynamicLoader and self.m_curHeroSpineObj then
    ...
    self.m_HeroSpineDynamicLoader:RecycleObjectByName(self.DefaultShowSpineName, self.m_curHeroSpineObj)
    self.m_curHeroSpineObj = nil
  end
end
...
function UIActivityTask14Base:LoadShowSpine()
  ...
  self.m_HeroSpineDynamicLoader:GetObjectByName(self.DefaultShowSpineName, function(nameStr, object)
    self:CheckRecycleSpine()
...
function UIActivityTask14Base:OnBtnClosePopClicked()
  UILuaHelper.SetActive(self.m_reward_pop, false)
  UILuaHelper.SetActive(self.m_btnClosePop, false)
end
...
function UIActivityTask14Base:IsFullScreen()
  return true
end

return UIActivityTask14Base
