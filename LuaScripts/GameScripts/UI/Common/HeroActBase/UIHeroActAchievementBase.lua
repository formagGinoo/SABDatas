local UIHeroActAchievementBase = class("UIHeroActAchievementBase", require("UI/Common/UIBase"))

function UIHeroActAchievementBase:AfterInit()
  UIHeroActAchievementBase.super.AfterInit(self)
  ...
  self.sAniName = "luoleilai_achievement_task_in"
end

function UIHeroActAchievementBase:OnActive()
  UIHeroActAchievementBase.super.OnActive(self)
  self:InitData()
  self:RefreshUI()
  self:AddEventListeners()
end

function UIHeroActAchievementBase:OnInactive()
  UIHeroActAchievementBase.super.OnInactive(self)
...
function UIHeroActAchievementBase:OnDestroy()
  UIHeroActAchievementBase.super.OnDestroy(self)
  if self.m_timer then
    TimeService:KillTimer(self.m_timer)
    self.m_timer = nil
  end
  if self.m_TaskListInfinityGrid1 then
    self.m_TaskListInfinityGrid1:dispose()
    self.m_TaskListInfinityGrid1 = nil
  end
  if self.m_TaskListInfinityGrid2 then
    self.m_TaskListInfinityGrid2:dispose()
    self.m_TaskListInfinityGrid2 = nil
  end
  if self.m_DailyTaskListInfinityGrid then
    self.m_DailyTaskListInfinityGrid:dispose()
    self.m_DailyTaskListInfinityGrid = nil
  end
end

function UIHeroActAchievementBase:AddEventListeners()
...
function UIHeroActAchievementBase:InitData()
...
function UIHeroActAchievementBase:RefreshUI()
...
function UIHeroActAchievementBase:CheckShowEnterAnim()
...
function UIHeroActAchievementBase:ShowItemListAnim()
  local grid = self.iCurSelectTab == self.TabEnum.DailyTask and self.m_TaskListInfinityGrid1 or self.m_TaskListInfinityGrid2
  local ItemList = grid:GetAllShownItemList()
  self.m_itemInitShowNum = #ItemList
  for i, Item in ipairs(ItemList) do
    ...
    self["ItemInitTimer" .. i] = TimeService:SetTimer(0.051 * (i - 1), 1, function()
      ...
      UILuaHelper.PlayAnimationByName(tempObj, self.sAniName)
    end)
  end
end

function UIHeroActAchievementBase:ShowCutDownTime()
...
function UIHeroActAchievementBase:OnBtngetallClicked()
  if self.iCurSelectTab == self.TabEnum.DailyTask then
    local flag = HeroActivityManager:CheckTaskCanReceive(self.iActId, self.iCurSelectTab == self.TabEnum.DailyTask)
    if flag then
...
function UIHeroActAchievementBase:OnBtnswitch1Clicked()
  if self.iCurSelectTab == self.TabEnum.DailyTask then
    return
  end
  self.iCurSelectTab = self.TabEnum.DailyTask
...
function UIHeroActAchievementBase:OnBtnswitch2Clicked()
...
function UIHeroActAchievementBase:OnBackClk()
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  self:CloseForm()
end

return UIHeroActAchievementBase
