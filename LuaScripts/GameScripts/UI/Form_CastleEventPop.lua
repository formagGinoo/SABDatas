...
function Form_CastleEventPop:OnActive()
  ...
  local showStoryType = self.m_csui.m_param.showStoryType
  ...
  local clientMessageId = showStoryType == CastleStoryManager.ShowStoryType.Playback and 48005 or 48003
  self.m_txt_content_Text.text = string.gsubNumberReplace(ConfigManager:GetClientMessageTextById(clientMessageId), cfg.m_mTitle)
  ...
  self.m_btn_confirm:SetActive(showStoryType ~= CastleStoryManager.ShowStoryType.Playback)
  self.m_btn_review:SetActive(showStoryType == CastleStoryManager.ShowStoryType.Playback)
end
...
function Form_CastleEventPop:OnBtnreviewClicked()
  if self.m_csui.m_param.callback then
    self.m_csui.m_param.callback()
    self.m_csui.m_param.callback = nil
  end
