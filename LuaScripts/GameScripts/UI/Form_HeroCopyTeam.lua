local Form_HeroCopyTeam = class("Form_HeroCopyTeam", require("UI/UIFrames/Form_HeroCopyTeamUI"))
local FormPlotMaxNum = HeroManager.FormPlotMaxNum
local FormTypeMaxNum = 6

function Form_HeroCopyTeam:SetInitParam(param)
end

function Form_HeroCopyTeam:AfterInit()
  self.super.AfterInit(self)
  self.FormTypeBase = nil
  self.m_isHideDevelopInfo = false
...
function Form_HeroCopyTeam:InitHeroWidgets()
...
function Form_HeroCopyTeam:OnActive()
...
function Form_HeroCopyTeam:OnInactive()
  self.super.OnInactive(self)
  self:RemoveAllEventListeners()
end

function Form_HeroCopyTeam:OnDestroy()
  self.super.OnDestroy(self)
end

function Form_HeroCopyTeam:FreshData()
  local params = self.m_csui.m_param
  if params then
    self.otherPlayerTeam = params.otherPlayerTeam or {}
    self.FormTypeBase = params.formType or HeroManager.TeamTypeBase.Default
    self.m_isHideDevelopInfo = params.isHideDevelopInfo
  end
  self.m_csui.m_param = nil
  self:FreshAllFormTeamData()
end

function Form_HeroCopyTeam:FreshAllFormTeamData()
...
function Form_HeroCopyTeam:FreshUI()
  self:FreshOtherPlayerTeamInfo()
  self:FreshMyTeamInfo()
end

function Form_HeroCopyTeam:FreshOtherPlayerTeamInfo()
  ...
  for i = 1, FormPlotMaxNum do
    ...
    if heroWidget then
      if serverData then
        ...
        local isHideDevelopInfo = self.m_isHideDevelopInfo == true
        heroWidget:SetHeroData(serverData, nil, isHideDevelopInfo, true, isHideDevelopInfo)
        local hero_data = HeroManager:GetHeroDataByID(serverData.iHeroId)
        if isHideDevelopInfo then
          heroWidget:SetObtainObjActive(hero_data == nil)
          heroWidget:SetStarNodeActive(false)
          heroWidget:SetCareerIconActive(hero_data ~= nil)
          heroWidget:SetImgLvBgObjActive(false)
        else
          ...
          heroWidget:SetImgLvBgObjActive(true)
        end
        heroWidget:SetHeroGrey(hero_data == nil, self.grayMat)
      else
        heroWidget:SetActive(false)
      end
    end
...
function Form_HeroCopyTeam:FreshMyTeamInfo()
...
function Form_HeroCopyTeam:AddEventListeners()
  self:addEventListener("eGameEvent_Hero_SetForm", handler(self, self.OnEventSetForm))
end

function Form_HeroCopyTeam:RemoveAllEventListeners()
  self:clearEventListener()
end

function Form_HeroCopyTeam:OnEventSetForm(param)
  if not param then
    return
  end
  ...
  StackPopup:Push(UIDefines.ID_FORM_HEROCOPYTIPS, {
    teamData = list,
...
function Form_HeroCopyTeam:FreshOneFromTeamData(formType, formData)
...
function Form_HeroCopyTeam:OnBtnarrowLClicked()
...
function Form_HeroCopyTeam:OnBtnarrowRClicked()
  local idx = self.curChooseIndex + 1
...
function Form_HeroCopyTeam:OnBtncopyClicked()
  local isSameServer = self:IsChooseHeroSameServer()
  if isSameServer then
    local serverDataList = self:GetCurFormHeroServerDataList()
    StackPopup:Push(UIDefines.ID_FORM_HEROCOPYTIPS, {
      teamData = serverDataList,
      teamIdx = self.curChooseIndex
    })
    return
  end
  ...
  if tips_id then
    utils.popUpDirectionsUI({
      tipsID = tips_id,
      func1 = function()
        HeroManager:ReqSetPreset(self.FormType, chooseHeroIDList, true)
      end
    })
  else
    HeroManager:ReqSetPreset(self.FormType, chooseHeroIDList, true)
...
function Form_HeroCopyTeam:IsChooseHeroSameServer()
...
function Form_HeroCopyTeam:GetCurFormHeroServerDataList()
  if not self.m_allFormTeams then
    return
  end
  if not self.FormType then
    return
  end
  local team = self.m_allFormTeams[self.FormType]
  if not team then
  ...
  local heroDataList = team.heroDataList
  if not heroDataList then
    return
  end
  local serverDataList = {}
  for _, heroData in ipairs(heroDataList) do
    if heroData.serverData then
      serverDataList[#serverDataList + 1] = heroData.serverData
    end
  end
  return serverDataList
end

function Form_HeroCopyTeam:OnBtnReturnClicked()
  self:CloseForm()
end

function Form_HeroCopyTeam:OnBtncancleClicked()
  self:CloseForm()
end

function Form_HeroCopyTeam:OnBtnCloseClicked()
  self:CloseForm()
end

function Form_HeroCopyTeam:IsOpenGuassianBlur()
  return true
end

local fullscreen = true
ActiveLuaUI("Form_HeroCopyTeam", Form_HeroCopyTeam)
