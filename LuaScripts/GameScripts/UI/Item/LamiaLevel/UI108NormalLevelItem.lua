...
local UI108NormalLevelItem = class("UI108NormalLevelItem", UIItemBase)
local posEnum = {
  [0] = Vector2.New(0, 60),
  [1] = Vector2.New(0, -140)
}

function UI108NormalLevelItem:OnInit()
...
function UI108NormalLevelItem:OnFreshData()
  ...
  if not utils.isNull(self.m_move_right_chilun) then
    UILuaHelper.SetActive(self.m_move_right_chilun, bIsCurrent)
  end
  if not utils.isNull(self.m_static_left_chilun) then
    UILuaHelper.SetActive(self.m_static_left_chilun, not bIsCurrent)
  end
  if not utils.isNull(self.m_static_right_chilun) then
    UILuaHelper.SetActive(self.m_static_right_chilun, not bIsCurrent)
  end
end

function UI108NormalLevelItem:FreshItemUI()
...
function UI108NormalLevelItem:ChangeChoose(isChoose)
  self.m_isChoose = isChoose
  self.m_itemData.isChoose = isChoose
  if not utils.isNull(self.m_bg_name_select) then
    UILuaHelper.SetActive(self.m_bg_name_select, isChoose)
  end
  local levelCfg = self.m_levelCfg
...
function UI108NormalLevelItem:OnBtnItemClk()
...
function UI108NormalLevelItem:GetCostItemNameStr()
...
function UI108NormalLevelItem:OnBtnRepeatClk()
  if not self.m_itemData then
    return
  end
  if not self.m_isUnlock then
...
function UI108NormalLevelItem:IsHaveEnoughTimes()
  local itemNum = self:GetCostItemNum() or 0
  return 0 < itemNum, itemNum
end

function UI108NormalLevelItem:GetCostItemNum()
...
function UI108NormalLevelItem:CheckShowUnlockAnim()
  if not self.m_isUnlock then
    return
  end
  local sKey = "UI108NormalLevelItem_UnlockAnim_" .. self.m_levelCfg.m_LevelID
  ...
  if not utils.isNull(self.m_bg_lock) then
    UILuaHelper.SetActive(self.m_bg_lock, true)
  end
