...
local UI109NormalLevelItem = class("UI109NormalLevelItem", UIItemBase)
local posEnum = {
  [0] = Vector2.New(0, 30),
  [1] = Vector2.New(0, -120)
}

function UI109NormalLevelItem:OnInit()
...
function UI109NormalLevelItem:OnFreshData()
  ...
  self.m_bg_unlock_hard:SetActive(false)
  self:FreshItemUI()
  ...
  if not utils.isNull(self.m_move_left_chilun) then
    self.m_move_left_chilun:SetActive(self.m_levelCfg.m_UIType == 1 and bIsCurrent)
  end
  if not utils.isNull(self.m_move_right_chilun) then
    self.m_move_right_chilun:SetActive(self.m_levelCfg.m_UIType == 1 and bIsCurrent)
  end
  if not utils.isNull(self.m_static_left_chilun) then
    self.m_static_left_chilun:SetActive(self.m_levelCfg.m_UIType == 1 and not bIsCurrent)
  end
  if not utils.isNull(self.m_static_right_chilun) then
    self.m_static_right_chilun:SetActive(self.m_levelCfg.m_UIType == 1 and not bIsCurrent)
  end
  if not utils.isNull(self.m_static_left_chilun_hard) then
    self.m_static_left_chilun_hard:SetActive(self.m_levelCfg.m_UIType == 2 and not bIsCurrent)
  end
  if not utils.isNull(self.m_static_right_chilun_hard) then
    self.m_static_right_chilun_hard:SetActive(self.m_levelCfg.m_UIType == 2 and not bIsCurrent)
  end
  if not utils.isNull(self.m_move_left_chilun_hard) then
    self.m_move_left_chilun_hard:SetActive(self.m_levelCfg.m_UIType == 2 and bIsCurrent)
  end
  if not utils.isNull(self.m_move_right_chilun_hard) then
    self.m_move_right_chilun_hard:SetActive(self.m_levelCfg.m_UIType == 2 and bIsCurrent)
  end
end

function UI109NormalLevelItem:FreshItemUI()
  ...
  if not utils.isNull(self.m_bg_lock) then
    UILuaHelper.SetActive(self.m_bg_lock, levelCfg.m_UIType == 1 and self.m_isUnlock ~= true)
  end
  if not utils.isNull(self.m_bg_lock_hard) then
    UILuaHelper.SetActive(self.m_bg_lock_hard, levelCfg.m_UIType ~= 1 and self.m_isUnlock ~= true)
  end
  if not utils.isNull(self.m_mask) then
    UILuaHelper.SetActive(self.m_mask, levelCfg.m_UIType == 1)
  end
  if not utils.isNull(self.m_mask_hard) then
    UILuaHelper.SetActive(self.m_mask_hard, levelCfg.m_UIType ~= 1)
  end
...
function UI109NormalLevelItem:ChangeChoose(isChoose)
...
function UI109NormalLevelItem:OnBtnItemClk()
...
function UI109NormalLevelItem:GetCostItemNameStr()
...
function UI109NormalLevelItem:OnBtnRepeatClk()
  if not self.m_itemData then
    return
  end
  if not self.m_isUnlock then
...
function UI109NormalLevelItem:OnBtnRewardClk()
  local clueCfg = HeroActivityManager:GetAct4ClueCfgByID(self.m_itemIndex)
  if clueCfg then
...
function UI109NormalLevelItem:IsHaveEnoughTimes()
  local itemNum = self:GetCostItemNum() or 0
  return 0 < itemNum, itemNum
end

function UI109NormalLevelItem:GetCostItemNum()
...
function UI109NormalLevelItem:CheckShowUnlockAnim()
  if not self.m_isUnlock then
    return
  end
  local sKey = "UI109NormalLevelItem_UnlockAnim_" .. self.m_levelCfg.m_LevelID
  local bIsShowAnim = LocalDataManager:GetIntSimple(sKey, 0)
  if bIsShowAnim == 1 then
    return
  end
  LocalDataManager:SetIntSimple(sKey, 1)
  local obj = self.m_levelCfg.m_UIType == 1 and self.m_bg_lock or self.m_bg_lock_hard
  if not utils.isNull(obj) then
    UILuaHelper.SetActive(obj, true)
  end
  local animName = self.m_levelCfg.m_UIType == 1 and "Activity109_DialogueMain_in" or "Activity109_DialogueMain_hard_in"
  UILuaHelper.PlayAnimationByName(self.m_itemTrans, animName)
  local fAniLength = UILuaHelper.GetAnimationLengthByName(self.m_itemTrans, animName)
