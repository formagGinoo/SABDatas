local Form_Activity106Tast14 = class("Form_Activity106Tast14", require("UI/UIFrames/Form_Activity106Tast14UI"))
local DefaultShowSpineName = "agatha_base"
local maxDailyReward = 5
local iUpActivityID = 1060
local RewardStatus = {
  CannotTake = 1,
  CanTake = 2,
  Taken = 3
...
function Form_Activity106Tast14:BindItemAndData()
  ...
  for i = 1, #keys do
    ...
    local curReward = mFinalRewardConfig[keys[i]]
    local tempTab = {
      reward = curReward,
      ...
      itemBox = self:CreateBoxItem(self["m_btn_box" .. i].transform:Find("c_common_itembox" .. i), curReward[1].iID, curReward[1].iNum),
...
function Form_Activity106Tast14:CreateBoxItem(itemObj, itemId, itemNum)
  if not itemObj then
    return nil
  end
  local common_item = self:createCommonItem(itemObj.gameObject)
  local processItemData = ResourceUtil:GetProcessRewardData({iID = itemId, iNum = itemNum})
  common_item:SetItemInfo(processItemData)
  return common_item
end

function Form_Activity106Tast14:OnActive()
...
function Form_Activity106Tast14:OnInactive()
...
function Form_Activity106Tast14:RemoveEventListeners()
...
function Form_Activity106Tast14:OnEventUpdateQuest(tParam)
...
function Form_Activity106Tast14:OnEventTakeQuestReward(sc)
...
function Form_Activity106Tast14:OnEventTakeDailyReward(sc)
...
function Form_Activity106Tast14:OnEventTakeFinalReward(sc)
  if sc.iActivityId ~= self.m_stActivity:getID() then
    return
  end
  utils.popUpRewardUI(sc.vReward)
...
function Form_Activity106Tast14:OnEventActivityReload()
  self:RefreshUI()
end

function Form_Activity106Tast14:RefreshUI()
...
function Form_Activity106Tast14:RefreshDailyTaskTab(bInit)
...
function Form_Activity106Tast14:GetDayQuestState(iDay)
  local bState
...
function Form_Activity106Tast14:RefreshDailyReward()
  ...
  self.m_bar_dailyreward_point1:SetActive(true)
  for i = 2, maxDailyReward + 1 do
    if self["m_bar_dailyreward_point" .. i] then
      self["m_bar_dailyreward_point" .. i]:SetActive(i <= iScoreCur + 1 and 0 < iScoreCur)
    end
  end
  self.m_bar_dailyreward_Image.fillAmount = iScoreCur / maxDailyReward
  self.item_list = self.item_list or {}
  local obj = self.m_pnl_item_dailyreward1
  obj:SetActive(true)
  local item = self.item_list[i]
  if item == nil then
    item = self:createCommonItem(obj)
  end
  local processData = ResourceUtil:GetProcessRewardData(stDailyRewardConfig.vReward[1])
...
function Form_Activity106Tast14:OnItemIconClicked(iID, iNum)
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  utils.openItemDetailPop({iID = iID, iNum = iNum})
end

function Form_Activity106Tast14:OnClaimDailyRewardClicked()
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  self.m_stActivity:RequestTakeDailyReward(self.iSelectedDay)
end

function Form_Activity106Tast14:RefreshFinalReward()
...
function Form_Activity106Tast14:RefreshPointReward()
  local iScoreCur = self.m_stActivity:GetScore()
  for key, item in pairs(self.pointItemData) do
    ...
    if item.itemStates ~= RewardStatus.Taken then
      if item.itemBox then
        item.itemBox:SetItemIconClickCB(function(itemID, num, itemCom)
          utils.openItemDetailPop({iID = itemID, iNum = num})
        end)
      else
    ...
    if item.isReGet then
      ...
      if item.itemBox then
        item.itemBox:SetItemIconClickCB(nil)
      end
      UILuaHelper.BindButtonClickManual(self, item.itemBtn, function()
...
function Form_Activity106Tast14:OnPointItemClick(item)
  self.m_stActivity:RequestTakeFinalReward(item.score)
end

function Form_Activity106Tast14:GetPointReward()
  self.m_stActivity:RequestTakeFinalReward(self.m_iFinalRewardScoreNext)
end

function Form_Activity106Tast14:RefreshRemainTime()
...
function Form_Activity106Tast14:refreshTaskLoopScroll(is_tab_change)
...
function Form_Activity106Tast14:updateScrollViewCell(index, cell_object, cell_data)
  ...
  LuaBehaviourUtil.setTextMeshPro(luaBehaviour, "txt_star_number1", stQuestConfig.iScore or "1")
  ...
  if stQuestConfig.iJump ~= 0 then
    ...
    LuaBehaviourUtil.setObjectVisible(luaBehaviour, "c_txt_doing", stQuestStatus.iState == MTTDProto.QuestState_Doing)
    if stQuestStatus.iState == MTTDProto.QuestState_Doing then
  ...
  LuaBehaviourUtil.setTextMeshPro(luaBehaviour, "c_txt_content", self.m_stActivity:getLangText(stQuestConfig.sName) .. "(" .. stepNum .. "/" .. stQuestConfig.iObjectiveCount .. ")")
  LuaBehaviourUtil.setObjectVisible(luaBehaviour, "c_img_break", stQuestConfig.iObjectiveType == MTTDProto.QuestCond_HeroBreakTo)
...
function Form_Activity106Tast14:OnBtngetallClicked()
...
function Form_Activity106Tast14:OnBtnsearchClicked()
  if self.heroId then
    StackPopup:Push(UIDefines.ID_FORM_HEROCHECK, {
      heroID = self.heroId
    })
...
function Form_Activity106Tast14:OnBtnfinalrewardreceiveClicked()
  self.m_stActivity:RequestTakeFinalReward(self.m_iFinalRewardScoreNext)
end

function Form_Activity106Tast14:OnTabItemClk(idx)
...
function Form_Activity106Tast14:Back()
  CS.GlobalManager.Instance:TriggerWwiseBGMState(2)
  self:CloseForm()
  self.iSelectedDay = nil
end

function Form_Activity106Tast14:OnDestroy()
  self.super.OnDestroy(self)
  self:CheckRecycleSpine(true)
end

function Form_Activity106Tast14:CheckRecycleSpine(isResetParam)
...
function Form_Activity106Tast14:LoadShowSpine()
  ...
  self.m_HeroSpineDynamicLoader:GetObjectByName(DefaultShowSpineName, function(nameStr, object)
    ...
    UILuaHelper.SpineResetInit(object)
...
function Form_Activity106Tast14:OnBtnClosePopClicked()
  UILuaHelper.SetActive(self.m_reward_pop, false)
  UILuaHelper.SetActive(self.m_btnClosePop, false)
end

function Form_Activity106Tast14:GetDownloadResourceExtra(tParam)
  local vPackage = {}
...
function Form_Activity106Tast14:IsFullScreen()
  return true
end
