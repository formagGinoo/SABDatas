...
function Form_EquipmentUpgrade:AfterInit()
  ...
  self.m_emptyRoot = goRoot.transform:Find("content_node/ui_common_empty").gameObject
...
function Form_EquipmentUpgrade:OnActive()
  ...
  self:CreateEquipIcon()
  self:RefreshEquipList()
  self:ExitFilterState()
...
function Form_EquipmentUpgrade:AddEventListeners()
  self:addEventListener("eGameEvent_Equip_AddExp", handler(self, self.OnAddExpCB))
...
function Form_EquipmentUpgrade:OnAddExpCB(param)
  ...
  if self.fxBarTimer then
    TimeService:KillTimer(self.fxBarTimer)
    self.fxBarTimer = nil
  end
  self.fxBarTimer = TimeService:SetTimer(1.5, 1, function()
...
function Form_EquipmentUpgrade:CreateEquipIcon()
...
function Form_EquipmentUpgrade:RefreshUpgradeEquipment(addExp)
  ...
  local newLv = equipData.iLevel + lvUp
  self.m_txt_lv_before_Text.text = equipData.iLevel
  self.m_txt_lv_after_Text.text = newLv
  ...
  self.m_lvUpNum = newLv
  self.m_txt_num_Text.text = tostring(newLv)
end

function Form_EquipmentUpgrade:OnNumStepperChange(reduce, isMax)
  if reduce then
    local afterReduce = self.m_lvUpNum - 1
    self.m_selItemList = {}
    ...
    self.m_isMaxLv = false
    self.autoSelNum = nil
    if afterReduce > self.m_equipLv then
      self:OnBtnautoblackClicked(afterReduce)
    end
  elseif isMax then
    self:OnBtnautoblackClicked(99)
  else
    self:OnBtnautoblackClicked()
  end
end
...
function Form_EquipmentUpgrade:RefreshAttrs(iBaseId, lv, addLv)
  ...
  local attrInfoList = {}
  if self.m_upgradeEquipData.iOverloadHero and self.m_upgradeEquipData.iOverloadHero ~= 0 then
...
function Form_EquipmentUpgrade:GeneratedListData(equipList, excludeItem)
  local dataList = {}
...
function Form_EquipmentUpgrade:SortEquipByLvUpDown2(equipList, filterDown)
  ...
  local function sortFun(data1, data2)
    ...
    if self.m_iFilterTabIndex == 1 then
      if quality1 == quality2 then
...
function Form_EquipmentUpgrade:GetUpgradeNextLevelNeedExp(equipData, selectedExp, levelTemplateID)
  local needExp = 0
  local _, lvUp = EquipManager:GetExpsCanUpgradeLvAndRemainingExp(equipData.iEquipUid, selectedExp)
  local levelTemplateID = levelTemplateID
  local upLv = 0
  local isMaxLv = false
  ...
  return needExp, isMaxLv
end

function Form_EquipmentUpgrade:SelectCanUpgradeNeedMaterialsByUid(equipUid, toLv)
  local equipData = EquipManager:GetEquipDataByID(equipUid)
  local selectedExp = self:GetSelectedItemsEXPs()
  local levelTemplateID = equipData.iOverloadHero == 0 and self.m_equipCfg.m_LevelTemplate or self.m_equipCfg.m_OverloadLevelTemplate
  local curExp = equipData.iExp or 0
  local needExp, isMaxLv = 0, false
  if not toLv then
    needExp, isMaxLv = self:GetUpgradeNextLevelNeedExp(equipData, selectedExp, levelTemplateID)
  else
    needExp, isMaxLv = EquipManager:GetEquipReachLvNeedExp(equipData.iLevel, levelTemplateID, toLv)
  end
  self.m_isMaxLv = isMaxLv
  needExp = needExp - curExp
  needExp = needExp - selectedExp
  if needExp <= 0 then
    return
  end
  local filterState = 0 < table.getn(self.m_chooseFilterType)
  if not filterState then
    needExp = self:UseMaterials(needExp)
    if needExp <= 0 then
      return
    end
  end
  local equipList, overConditionTips = self:GetUnSelectedEquips(filterState)
...
function Form_EquipmentUpgrade:UseMaterials(needExp)
...
function Form_EquipmentUpgrade:GetUnSelectedEquips(filterState)
  local list = {}
  local overConditionTips
  local equipList = filterState and self.m_selFilterEquipList or EquipManager:GetUnEquippedEquipsById(self.m_upgradeEquipData.iEquipUid)
  local equipCfg = self.m_equipCfg
...
function Form_EquipmentUpgrade:MarkSelectedMaterials()
  for m, n in ipairs(self.m_allSortedItemDataList) do
    self.m_EquipListInfinityGrid:SetUpGradeNum(m, 0)
  end
  for i, v in pairs(self.m_selItemList) do
    for m, n in ipairs(self.m_allSortedItemDataList) do
      if i == n.iID then
...
function Form_EquipmentUpgrade:OnBtnreduceClicked()
  self:OnNumStepperChange(true)
end

function Form_EquipmentUpgrade:OnBtnincreaseClicked()
  self:OnNumStepperChange(false, false)
end

function Form_EquipmentUpgrade:OnBtnmaxClicked()
  self:OnNumStepperChange(false, true)
end

function Form_EquipmentUpgrade:OnBtnautoblackClicked(toLv)
  if self.m_isMaxLv then
    StackPopup:Push(UIDefines.ID_FORM_COMMON_TOAST, 30022)
    return
  end
  local overConditionTips = self:SelectCanUpgradeNeedMaterialsByUid(self.m_upgradeEquipData.iEquipUid, toLv)
...
function Form_EquipmentUpgrade:OnBtntitletipsClicked()
  utils.popUpDirectionsUI({tipsID = 1254})
end

function Form_EquipmentUpgrade:OnBtnFilterClicked()
  local function chooseBackFun(filterData)
    self.m_chooseFilterType = filterData
    
    local count = table.getn(self.m_selFilterEquipList)
    local equips = EquipManager:FilterUpgradeEquipByConditions(self.m_equipDataList, filterData)
    self.m_selFilterEquipList = equips
    if count >= table.getn(equips) then
  ...
  utils.openFormBagEquipFilter(self.m_allSortedItemDataList, self.m_btn_Filter.transform, {x = 1, y = 1}, {x = -35, y = 130}, chooseBackFun, self.m_chooseFilterType)
end

function Form_EquipmentUpgrade:RefreshFilterUI()
  self.m_filter_select:SetActive(table.getn(self.m_chooseFilterType) > 0)
  local addExp = self:GetSelectedItemsEXPs()
  self:RefreshUpgradeEquipment(addExp)
  if table.getn(self.m_chooseFilterType) == 0 then
    self:RefreshEquipList()
  ...
  self.m_allSortedItemDataList = self:GeneratedListData(self.m_selFilterEquipList, true)
  if 0 < #self.m_allSortedItemDataList then
    self.m_equip_list:SetActive(true)
    self.m_EquipListInfinityGrid:ShowItemList(self.m_allSortedItemDataList)
    self.m_EquipListInfinityGrid:LocateTo(0)
    self.m_emptyRoot:SetActive(false)
...
function Form_EquipmentUpgrade:ExitFilterState()
  self.m_chooseFilterType = {}
  self.m_selFilterEquipList = {}
  self.m_filter_select:SetActive(table.getn(self.m_chooseFilterType) > 0)
end
