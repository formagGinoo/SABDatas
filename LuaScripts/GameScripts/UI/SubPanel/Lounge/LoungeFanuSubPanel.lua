...
local LoungeFanuSubPanel = class("LoungeFanuSubPanel", UISubPanelBase)
local __DefaultControl = "eye_control"
local __ShowBondBtn = 1

function LoungeFanuSubPanel:OnInit()
  print("LoungeFanuSubPanel OnInit")
...
function LoungeFanuSubPanel:OnActive()
...
function LoungeFanuSubPanel:OnInactive()
  self:KillTimer()
  if self.m_spineClick then
...
function LoungeFanuSubPanel:OnOpen()
...
function LoungeFanuSubPanel:GenerateGestureParam()
...
function LoungeFanuSubPanel:OnSelectedHero()
  print("LoungeFanuSubPanel OnSelectedHero")
...
function LoungeFanuSubPanel:ShowHeroSpine(showSpineStr)
  print("LoungeFanuSubPanel ShowHeroSpine", showSpineStr)
...
function LoungeFanuSubPanel:OnLoadSpineBack(showSpineStr, reBind)
...
function LoungeFanuSubPanel:SpinePlayAnimByActionName(actionName, cfg, callBack, changeAction, reverse, cycleChangeAction, preCallback)
...
function LoungeFanuSubPanel:ShowPlotTextByStr(str, voice)
  if str and str ~= "" then
    self:ShowPlotPanel()
...
function LoungeFanuSubPanel:OnCloseMiniGame()
  self.m_stopSpineIdle = false
  self:SpinePlayAnimByActionName()
end

function LoungeFanuSubPanel:OnCompleteMiniGame()
  self:RefreshHeroLoungeData()
  self.m_longPressState = nil
  self:CompleteMiniGameAction(1)
end

function LoungeFanuSubPanel:CompleteMiniGameAction(delayTime)
...
function LoungeFanuSubPanel:JumpToIdle03()
...
function LoungeFanuSubPanel:OnCompleteSpecialPart(iPart)
  if not iPart then
    return
  end
...
function LoungeFanuSubPanel:OpenMiniGame(cfg)
...
function LoungeFanuSubPanel:ResetGame()
...
function LoungeFanuSubPanel:DestroyAndUnloadSpine()
...
function LoungeFanuSubPanel:ShowPlotPanel()
...
function LoungeFanuSubPanel:RefreshHeroLoungeData()
...
function LoungeFanuSubPanel:GetChangeIdleTime()
...
function LoungeFanuSubPanel:ShowPlotText(cfg)
  if utils.isNull(self.m_pnl_dialogue) then
    return
  end
...
function LoungeFanuSubPanel:OnUpdate(dt)
  self:OnLongPress(dt)
  self:CheckTips(dt)
...
function LoungeFanuSubPanel:AddEventListeners()
...
function LoungeFanuSubPanel:RemoveEventListeners()
  self:clearEventListener()
end

function LoungeFanuSubPanel:RefreshUI()
...
function LoungeFanuSubPanel:ShowALlUI(flag)
  self.m_parentLua:ShowALlUI(flag)
end

function LoungeFanuSubPanel:OnInteractionRefreshUI(stData)
  self:RefreshHeroLoungeData()
end
...
function LoungeFanuSubPanel:OnCompleteStampMark(realMark)
  if utils.isNull(self.m_parentLua.m_common_top_back) then
    return
  end
  LoungeManager:SetClientLoungeState(self.m_curLoungeHeroId, LoungeManager.LoungeBondStatus.Bond)
  self.m_longPressState = nil
  self:RefreshHeroLoungeData()
  UILuaHelper.SetActive(self.m_parentLua.m_common_top_back, true)
  UILuaHelper.SetActive(self.m_pnl_btn, true)
  self:ShowBondBtn(false)
  if not realMark then
...
function LoungeFanuSubPanel:CheckReqAttractLoungeBond()
...
function LoungeFanuSubPanel:FreshTimerTip()
  self.fTipTimer = 0
  self.bIsCheckTips = true
end

function LoungeFanuSubPanel:OnSpineClick(name, localPos, touchType)
  self.m_iTimeDurationOneSecond = 0
...
function LoungeFanuSubPanel:ResetControlBone()
  if self.m_spineClick then
    self.m_spineClick:ChangeControlBone(__DefaultControl)
  end
end

function LoungeFanuSubPanel:OnLongPressUp()
...
function LoungeFanuSubPanel:OnLongPressDown()
  if utils.isNull(self.m_parentLua.m_common_top_back) then
    return
  end
  self.m_stopRandomIdle = true
  UILuaHelper.SetActive(self.m_pnl_contract, true)
  UILuaHelper.SetActive(self.m_parentLua.m_common_top_back, false)
...
function LoungeFanuSubPanel:OnLongPress(dt)
...
function LoungeFanuSubPanel:KillTimer()
  if self.m_completeMiniGameTimer then
    TimeService:KillTimer(self.m_completeMiniGameTimer)
    self.m_completeMiniGameTimer = nil
  end
...
function LoungeFanuSubPanel:SetCheckTipsFlag(bCheck)
  self.bIsCheckTips = bCheck
end

function LoungeFanuSubPanel:OnBtnhelpClicked()
  StackPopup:Push(UIDefines.ID_FORM_LOUNGEGUIDEPOP2, {
    heroId = self.m_curLoungeHeroId
  })
end

function LoungeFanuSubPanel:OnDestroy()
