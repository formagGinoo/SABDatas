...
function rpcs:Net_Connect(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Net_Connect_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Net_Connect timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Net_Connect_SC)
...
function rpcs:Net_ConnectNoCB(sdpMsg, timeout, retryCount)
  self:Net_Connect(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Net_Connect_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Net_Connect(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Net_Connect(cb, tag)
  if cb == nil then
    log.info("ListenNet_Connect cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Net_Connect_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Net_Connect_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Net_Connect_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Net_Connect_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Net_Connect(handler)
