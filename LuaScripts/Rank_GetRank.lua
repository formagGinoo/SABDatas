...
function rpcs:Rank_GetRank(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Rank_GetRank_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Rank_GetRank timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Rank_GetRank_SC)
...
function rpcs:Rank_GetRankNoCB(sdpMsg, timeout, retryCount)
  self:Rank_GetRank(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Rank_GetRank_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Rank_GetRank(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Rank_GetRank(cb, tag)
  if cb == nil then
    log.info("ListenRank_GetRank cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Rank_GetRank_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Rank_GetRank_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Rank_GetRank_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Rank_GetRank_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Rank_GetRank(handler)
