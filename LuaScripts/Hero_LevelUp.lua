...
function rpcs:Hero_LevelUp(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Hero_LevelUp_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Hero_LevelUp timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Hero_LevelUp_SC)
...
function rpcs:Hero_LevelUpNoCB(sdpMsg, timeout, retryCount)
  self:Hero_LevelUp(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Hero_LevelUp_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Hero_LevelUp(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Hero_LevelUp(cb, tag)
  if cb == nil then
    log.info("ListenHero_LevelUp cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Hero_LevelUp_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Hero_LevelUp_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Hero_LevelUp_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Hero_LevelUp_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Hero_LevelUp(handler)
