...
function rpcs:Stage_MopUp(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Stage_MopUp_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Stage_MopUp timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Stage_MopUp_SC)
...
function rpcs:Stage_MopUpNoCB(sdpMsg, timeout, retryCount)
  self:Stage_MopUp(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Stage_MopUp_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Stage_MopUp(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Stage_MopUp(cb, tag)
  if cb == nil then
    log.info("ListenStage_MopUp cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Stage_MopUp_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Stage_MopUp_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Stage_MopUp_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Stage_MopUp_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Stage_MopUp(handler)
