...
function rpcs:Legacy_Swap(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Legacy_Swap_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Legacy_Swap timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Legacy_Swap_SC)
...
function rpcs:Legacy_SwapNoCB(sdpMsg, timeout, retryCount)
  self:Legacy_Swap(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Legacy_Swap_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Legacy_Swap(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Legacy_Swap(cb, tag)
  if cb == nil then
    log.info("ListenLegacy_Swap cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Legacy_Swap_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Legacy_Swap_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Legacy_Swap_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Legacy_Swap_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Legacy_Swap(handler)
