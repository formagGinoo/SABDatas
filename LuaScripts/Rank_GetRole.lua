...
function rpcs:Rank_GetRole(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Rank_GetRole_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Rank_GetRole timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Rank_GetRole_SC)
...
function rpcs:Rank_GetRoleNoCB(sdpMsg, timeout, retryCount)
  self:Rank_GetRole(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Rank_GetRole_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Rank_GetRole(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Rank_GetRole(cb, tag)
  if cb == nil then
    log.info("ListenRank_GetRole cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Rank_GetRole_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Rank_GetRole_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Rank_GetRole_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Rank_GetRole_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Rank_GetRole(handler)
