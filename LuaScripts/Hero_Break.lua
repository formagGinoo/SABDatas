...
function rpcs:Hero_Break(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Hero_Break_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Hero_Break timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Hero_Break_SC)
...
function rpcs:Hero_BreakNoCB(sdpMsg, timeout, retryCount)
  self:Hero_Break(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Hero_Break_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Hero_Break(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Hero_Break(cb, tag)
  if cb == nil then
    log.info("ListenHero_Break cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Hero_Break_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Hero_Break_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Hero_Break_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Hero_Break_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Hero_Break(handler)
