...
function rpcs:Shop_GetShop(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Shop_GetShop_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Shop_GetShop timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Shop_GetShop_SC)
...
function rpcs:Shop_GetShopNoCB(sdpMsg, timeout, retryCount)
  self:Shop_GetShop(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Shop_GetShop_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Shop_GetShop(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Shop_GetShop(cb, tag)
  if cb == nil then
    log.info("ListenShop_GetShop cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Shop_GetShop_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Shop_GetShop_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Shop_GetShop_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Shop_GetShop_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Shop_GetShop(handler)
