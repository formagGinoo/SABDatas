...
function rpcs:Sns_Share(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Sns_Share_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Sns_Share timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Sns_Share_SC)
...
function rpcs:Sns_ShareNoCB(sdpMsg, timeout, retryCount)
  self:Sns_Share(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Sns_Share_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Sns_Share(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Sns_Share(cb, tag)
  if cb == nil then
    log.info("ListenSns_Share cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Sns_Share_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Sns_Share_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Sns_Share_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Sns_Share_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Sns_Share(handler)
