...
function rpcs:Mail_DelMail(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Mail_DelMail_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Mail_DelMail timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Mail_DelMail_SC)
...
function rpcs:Mail_DelMailNoCB(sdpMsg, timeout, retryCount)
  self:Mail_DelMail(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Mail_DelMail_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Mail_DelMail(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Mail_DelMail(cb, tag)
  if cb == nil then
    log.info("ListenMail_DelMail cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Mail_DelMail_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Mail_DelMail_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Mail_DelMail_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Mail_DelMail_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Mail_DelMail(handler)
