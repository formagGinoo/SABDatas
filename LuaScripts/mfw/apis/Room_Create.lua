...
function rpcs:Room_Create(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Room_Create_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Room_Create timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Room_Create_SC)
...
function rpcs:Room_CreateNoCB(sdpMsg, timeout, retryCount)
  self:Room_Create(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Room_Create_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Room_Create(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Room_Create(cb, tag)
  if cb == nil then
    log.info("ListenRoom_Create cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Room_Create_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Room_Create_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Room_Create_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Room_Create_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Room_Create(handler)
