...
function rpcs:Form_SetForm(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Form_SetForm_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Form_SetForm timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Form_SetForm_SC)
...
function rpcs:Form_SetFormNoCB(sdpMsg, timeout, retryCount)
  self:Form_SetForm(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Form_SetForm_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Form_SetForm(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Form_SetForm(cb, tag)
  if cb == nil then
    log.info("ListenForm_SetForm cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Form_SetForm_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Form_SetForm_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Form_SetForm_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Form_SetForm_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Form_SetForm(handler)
