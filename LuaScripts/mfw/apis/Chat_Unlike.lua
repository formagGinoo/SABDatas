...
function rpcs:Chat_Unlike(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Chat_Unlike_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Chat_Unlike timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Chat_Unlike_SC)
...
function rpcs:Chat_UnlikeNoCB(sdpMsg, timeout, retryCount)
  self:Chat_Unlike(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Chat_Unlike_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Chat_Unlike(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Chat_Unlike(cb, tag)
  if cb == nil then
    log.info("ListenChat_Unlike cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Chat_Unlike_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Chat_Unlike_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Chat_Unlike_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Chat_Unlike_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Chat_Unlike(handler)
