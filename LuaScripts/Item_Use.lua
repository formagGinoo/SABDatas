...
function rpcs:Item_Use(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Item_Use_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Item_Use timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Item_Use_SC)
...
function rpcs:Item_UseNoCB(sdpMsg, timeout, retryCount)
  self:Item_Use(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Item_Use_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Item_Use(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Item_Use(cb, tag)
  if cb == nil then
    log.info("ListenItem_Use cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Item_Use_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Item_Use_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Item_Use_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Item_Use_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Item_Use(handler)
