...
function rpcs:Room_Kick(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Room_Kick_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Room_Kick timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Room_Kick_SC)
...
function rpcs:Room_KickNoCB(sdpMsg, timeout, retryCount)
  self:Room_Kick(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Room_Kick_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Room_Kick(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Room_Kick(cb, tag)
  if cb == nil then
    log.info("ListenRoom_Kick cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Room_Kick_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Room_Kick_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Room_Kick_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Room_Kick_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Room_Kick(handler)
