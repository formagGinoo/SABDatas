...
function rpcs:Form_GetForm(sdpMsg, cbSuccess, cbFailed, cbTimeout, timeoutTime, retryCount, lockscreenTick)
  local client = self.CsSession:GetNetClient(CsNet.NetClientType.Game)
  if client == nil then
    log.error("Request Client Game NULL!")
    return
  end
  local messageId = MTTDProto.CmdId_Form_GetForm_CS
  ...
  if cbSuccess ~= nil or cbFailed ~= nil or cbTimeout ~= nil then
    ...
    function cb.callback(rec, msg, bt)
      if msg == nil then
        log.info("Form_GetForm timeout: " .. tostring(rec.timeoutTick))
      ...
      if msg.rspcode == 0 then
        local sc = sdp.unpack(bt, MTTDProto.Cmd_Form_GetForm_SC)
...
function rpcs:Form_GetFormNoCB(sdpMsg, timeout, retryCount)
  self:Form_GetForm(sdpMsg, nil, nil, nil, timeout, retryCount)
end

rpcs.Form_GetForm_Task = awrap(function(self, sdpMsg, timeout, retryCount, callback)
  self:Form_GetForm(sdpMsg, function(sc, msg)
...
function rpcs:Listen_Form_GetForm(cb, tag)
  if cb == nil then
    log.info("ListenForm_GetForm cb == nil")
    return
  end
  local messageId = -MTTDProto.CmdId_Form_GetForm_SC
  return self.CsSession:Listen(messageId, function(msg)
    if msg.rspcode == 0 then
      local sc = sdp.unpack(msg.bt:ToBytes(), MTTDProto.Cmd_Form_GetForm_SC)
      cb(true, sc, msg)
    else
      cb(false, nil, msg)
    end
...
function rpcs:RemoveListen_Form_GetForm_ByTag(tag)
  local messageId = -MTTDProto.CmdId_Form_GetForm_SC
  self.CsSession:RemoveListenByTagAndId(tag, messageId)
end

function rpcs:RemoveListen_Form_GetForm(handler)
